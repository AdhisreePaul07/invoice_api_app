const financeThemeValue = (name, fallback) => {
	const value = getComputedStyle(document.documentElement).getPropertyValue(name).trim();
	return value || fallback;
};

const financeThemeRgba = (name, alpha, fallback) =>
	`rgba(${financeThemeValue(name, fallback)}, ${alpha})`;

const financePrimaryScale = (...alphas) =>
	alphas.map((alpha) => financeThemeRgba('--app-primary-rgb', alpha, '13, 155, 108'));
...
function expenseChartConfig() {
	...
	const expenseChartColors = financePrimaryScale(1, 0.58, 0.28, 0.12);
