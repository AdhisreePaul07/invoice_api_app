...
// Universal chart initializer
const initChart = (selector, config) => {
    const el = $get(selector);
    if (!el) return console.warn(`Chart skipped: ${selector} not found`);
    try {
        const chart = new ApexCharts(el, config);
        chart.render();
    } catch (err) {
        console.error(`Chart error at ${selector}:`, err);
    }
};
...
document.addEventListener("DOMContentLoaded", () => {
    initChart("#chartEmployee", employeeChartConfig);
    initChart("#chartAttendanceRate", attendanceChartConfig);
    initChart("#chartPayrollSummary", payrollChartConfig);
    initChart("#averageTeamKPI", kpiChartConfig);
    initChart("#chartDailySalesStates", heatmapConfig);
    initChart("#chartStocksPrices", candlestickConfig);
});