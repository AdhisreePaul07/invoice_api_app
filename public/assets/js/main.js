(() => {
	...
	function initTooltips() {
		...
		const tooltipTriggerList = document.querySelectorAll('[data-app-toggle="tooltip"]');
	...
	function initPopover() {
		if (!window.bootstrap) return;
		var popoverTriggerList = [].slice.call(document.querySelectorAll('[data-app-toggle="popover"]'))
	...
	const ThemeSwitcher = () => {
		...
		const syncClienetBrandLogos = (theme) => {
		  const logoSrc = theme === 'dark'
			? '/assets/images/clienet-logo-white.png'
			: '/assets/images/clienet-logo.png';
		  const brandSelector = [
			'.app-side-brands .navbar-brand-text img',
			'a[aria-label="Clienet Logo"] img',
			'a[aria-label="Clienet logo"] img'
		  ].join(', ');

		  document.querySelectorAll(brandSelector).forEach((img) => {
			img.classList.remove('visible-light', 'visible-dark');
		...
		const setTheme = (theme) => {
		  docEl.setAttribute('data-app-theme', theme);
		  syncClienetBrandLogos(theme);
		};
		...
		$(document).ready(function () {
		  ...
		  $('.theme-btn').on('click', function () {
			...
			let currentTheme = docEl.getAttribute('data-app-theme');
